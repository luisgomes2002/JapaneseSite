    ...
    const timers = data.map((_, index) =>
      setInterval(() => {
        setIndicesAtuais((indices) =>
          indices.map((indiceAtual, i) => (i === index ? (indiceAtual + 1) % CreatorsInfo[i].imgC.length : indiceAtual))
        )
      }, 2000)
    )
                  ...
                  <div className='creators-info--space' key={info.id}>
                    <div className='info-creators'>
                      <div className='img-space-creators'>
                        ...
                        <img src={info.imgC[indiceAtual % info.imgC.length]} alt='' />
                      </div>
                      <div className='info-text-area'>
                        <Link to={info.link[2]}><h1>{info.name}</h1></Link>
                        <h2>{info.text}</h2>
                        <p>{info.sub}</p>
                      </div>
                      <div className='link-area'>
                        {info.link.map((link, i) => link && (
                          <Link key={i} to={link} target='react/jsx-no-target-blank'>
                            <i className={`fab fa-${i === 0 ? 'twitch' : i === 1 ? 'instagram' : i === 2 ? 'youtube' : 'twitter'}`}></i>
                          </Link>
                        ))}
                      </div>
                    </div>
...
const CreatorsArea = () => {
  ...
  return (
    <div className='content-creators'>
      ...
      {currentLevel === 2 && <div><Estudo /></div>}
      {currentLevel === 1 && <div><Entretenimento /></div>}
    </div>
  )
}
